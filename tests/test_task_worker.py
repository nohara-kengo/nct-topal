...
def test_worker_create(mock_classify, mock_ssm, mock_generate, mock_create, mock_notify):
    ...
    assert "作成しました" in mock_notify.call_args[0][0]
    assert "NOHARATEST-99" in mock_notify.call_args[0][0]
...
def test_worker_update(mock_classify, mock_ssm, mock_update, mock_notify):
    ...
    assert result["processed"] == 1
    mock_notify.assert_called_once()
    assert "更新しました" in mock_notify.call_args[0][0]
...
def test_worker_no_project_key(mock_classify, mock_notify):
    ...
    assert result["processed"] == 1
    mock_notify.assert_called_once()
    assert "プロジェクトキー" in mock_notify.call_args[0][0]
