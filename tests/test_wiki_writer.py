...
EXISTING_WIKIS = [
    {"id": 10, "name": "日次レポート/2026-03-25/全体"},
    {"id": 11, "name": "日次レポート/2026-03-24/全体"},
    {"id": 12, "name": "日次レポート/2026-03-24/野原太郎"},
]
...
def test_update_existing_wiki(mock_get, mock_create, mock_update):
    mock_update.return_value = {"id": 10, "name": "日次レポート/2026-03-25/全体"}
    pages = [{"name": "日次レポート/2026-03-25/全体", "content": "# 更新"}]

    results = wiki_writer.write_daily_report("NOHARATEST", "2026/03/25", pages)

    assert len(results) == 1
    mock_update.assert_called_once_with(10, "日次レポート/2026-03-25/全体", "# 更新", "NOHARATEST")
...
def test_create_new_wiki(mock_get, mock_create, mock_update):
    mock_create.return_value = {"id": 30, "name": "日次レポート/2026-03-26/全体"}
    pages = [{"name": "日次レポート/2026-03-26/全体", "content": "# 新規"}]

    results = wiki_writer.write_daily_report("NOHARATEST", "2026/03/26", pages)

    assert len(results) == 1
    mock_create.assert_called_once_with("NOHARATEST", "日次レポート/2026-03-26/全体", "# 新規")
...
def test_mixed_create_and_update(mock_get, mock_create, mock_update):
    mock_update.return_value = {"id": 10, "name": "日次レポート/2026-03-25/全体"}
    mock_create.return_value = {"id": 30, "name": "日次レポート/2026-03-25/野原太郎"}
    pages = [
        {"name": "日次レポート/2026-03-25/全体", "content": "# 全体"},
        {"name": "日次レポート/2026-03-25/野原太郎", "content": "# 野原"},
    ]
...
def test_empty_existing_wikis(mock_get, mock_create, mock_update):
    mock_create.return_value = {"id": 40, "name": "日次レポート/2026-03-25/全体"}
    pages = [{"name": "日次レポート/2026-03-25/全体", "content": "# テスト"}]
...
def test_fetch_prev_wikis(mock_get_wikis, mock_fetch):
    ...
    assert len(result) == 2
    assert "日次レポート/2026-03-24/全体" in result
    assert "日次レポート/2026-03-24/野原太郎" in result
    assert result["日次レポート/2026-03-24/全体"] == "content-11"
    assert result["日次レポート/2026-03-24/野原太郎"] == "content-12"
