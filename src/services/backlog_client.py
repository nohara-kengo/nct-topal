...
def get_issues(project_key: str, **filters) -> list[dict]:
    """課題一覧を取得する。

    Args:
        project_key: Backlogプロジェクトキー
        **filters: 検索フィルタ（statusId[], assigneeId[]等）
    ...
    params = {"apiKey": api_key, "projectId[]": project["id"], "count": 100}
    params.update(filters)
    url = f"{space_url}/api/v2/issues"
    resp = _request_with_retry("GET", url, params=params, timeout=10)
    return resp.json()


def get_wikis(project_key: str) -> list[dict]:
    """Wiki一覧を取得する。
    ...
    resp = _request_with_retry("GET", url, params={"apiKey": api_key, "projectIdOrKey": project["id"]}, timeout=10)
    return resp.json()


def create_wiki(project_key: str, name: str, content: str) -> dict:
    """Wikiページを作成する。

    Args:
        project_key: Backlogプロジェクトキー
        name: Wikiページ名
        content: Wikiページ内容（Markdown）
    ...
    project = get_project(project_key)
    url = f"{space_url}/api/v2/wikis"
    data = {"projectId": project["id"], "name": name, "content": content}
    resp = _request_with_retry("POST", url, params={"apiKey": api_key}, data=data, timeout=10)
    return resp.json()


def update_wiki(wiki_id: int, name: str, content: str, project_key: str) -> dict:
    """Wikiページを更新する。
    ...
    """
    space_url, api_key = _get_auth_params(project_key)
    url = f"{space_url}/api/v2/wikis/{wiki_id}"
